        ...
        if st.session_state.auth_mode == "login":
            ...
            if submit_clicked:
                try:
                    ...
                    res = requests.post(f"{AUTH_END_POINT}/token", data=payload, timeout=5, allow_redirects=True)
                ...
                if st.form_submit_button("Register", use_container_width=True):
                    ...
                    if not all(fields):
                        ...
                        try:
                            ...
                            check_res = requests.get(f"{USER_END_POINT}/check-username/{username}", timeout=5, allow_redirects=True)
                            if check_res.status_code == 200:
                                if check_res.json().get("is_taken"):
                                    ...
                                    reg_res = requests.post(f"{USER_END_POINT}", json=payload, timeout=5, allow_redirects=True)
